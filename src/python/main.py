		...
		with open(path) as f:
			...
			for i in ms:
				for j in ns:
					...
					## no se esta guardando la solution bien 
					result.setSolutions(bestError,solutions,time)
					result.saveState()
					...
					for k in breakpoints_list:
						print(f"fuerza bruta en {k} breakpoints")
						if k < 5:
							bestError,solutions,time = fuerzaBruta(i,j,k,instance)
							errors_listFB.append(time)
						else:
							errors_listFB.append(errors_listFB[2])
							

						print(f"back tracking en {k} breakpoints")
						bestError,solutions,time = backTracking(i,j,k, instance)
						errors_listBT.append(time)
						print(f"programacion dinamica en {k} breakpoints")
					...
					comparacion_errores(errors_list, breakpoints_list, instance, dataName, i ,j)
